...
class DataTableViewController: UITableViewController {
    ...
    @objc func managedObjectContextDidSave(notification: NSNotification) {
        ...
        if let inserts = userInfo[NSInsertedObjectsKey] as? Set<NSManagedObject>, inserts.count > 20 {
            print("BINGOOOOO")
        }
    }
    ...
    private func fetchAllObjects() {
        ...
        /*
         ...
         let request: NSFetchRequest<NSFetchRequestResult> = Company.fetchRequest()
         guard let result = try? coreDataStack.backgroundContext.fetch(request) else { return }//coreDataStack.managedObjectContex.fetch(request) as? [Company] else { return }
         //            if result != nil {
         allCompanies = result as! [Company]
